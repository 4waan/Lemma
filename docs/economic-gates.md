...
| 4 | **Economic probe** (before the paid path is finished) | Two or three exploratory control runs and one hand-applied treatment run per candidate release give a rough `S / C`. It must clear both the sale rule and `S >= 0.357 C`, plus a price floor that covers `g`. | Either threshold fails. Pick larger or more failure-prone integration tasks before spending on payments. | N |
...
8. **Deterministic ranking at catalog scale.** When several releases match, rank by a published total order: decision class, evidence present, pass rate, price, semver descending, release id.
