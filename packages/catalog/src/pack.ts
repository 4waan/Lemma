...
export function packPayload(releaseDir: string): PatchBundle {
  ...
  assertExactly(needsContent, listFilesRecursive(releaseDir, `${PAYLOAD_DIR}/files`), "files");
  assertExactly(needsBase, listFilesRecursive(releaseDir, `${PAYLOAD_DIR}/base`), "base");

  const files = ops.files
    .map((f) => ({
      path: f.path,
      op: f.op,
      baseDigest: f.op === "add" ? null : fileDigest(readBytes(releaseDir, `${PAYLOAD_DIR}/base/${f.path}`)),
      content: f.op === "delete" ? null : readText(releaseDir, `${PAYLOAD_DIR}/files/${f.path}`),
    }))
    .sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));

  return PatchBundle.parse({ schemaVersion: "1", files, dependencies: ops.dependencies, devDependencies: ops.devDependencies });
}
...
function assertExactly(expected: ReadonlySet<string>, present: readonly string[], dir: string): void {
  ...
  if (missing.length > 0) throw new Error(`${PAYLOAD_DIR}/${dir}: missing ${missing.join(", ")}`);
  if (extra.length > 0) throw new Error(`${PAYLOAD_DIR}/${dir}: not named in ops.json: ${extra.join(", ")}`);
}
